const workercode = () => {
    var onmessage = function(msg) {
        if (msg.data.type === 0) {
            ...
            var learn   = msg.data.data.learn;
            var test    = msg.data.data.test;
            var samples = msg.data.data.samples;
            var confidences = msg.data.data.confidences;

            var result;
            result = ooc_calculation(learn, test, samples, confidences, handleProgressInfo);
            postMessage({type: 2, result: result});
        }
    }

    function handleProgressInfo(progress) {
        var progressInfo = progress;
        postMessage({type: 1, result: progress});
    }
    ...
    function ooc_calculation(set_0, set_1, samples, confidences, progressInfo) {
        ...
        return {icdf_learn: icdf_0, icdf_window: icdf_1, quants: quants, layers: layers, ooc: ooc_result};
    }
    
};

let code = workercode.toString();
code = code.substring(code.indexOf("{")+4, code.lastIndexOf("}"));
